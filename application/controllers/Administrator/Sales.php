...
class Sales extends CI_Controller
{
    ...
    public function get_serial_report()
    {
        ...
        $transferReport =  $this->db->query("
                SELECT
                    'transfer Invoice' as invoiceNo,
                    tm.transfer_date as transferDate,
                    sn.ps_id,
                    sn.ps_serial_number
                from  tbl_product_serial_numbers sn
                left join tbl_transferdetails td on td.transferdetails_id = sn.transfer_details_id
                left join tbl_transfermaster tm on tm.transfer_id = td.transfer_id
                where sn.ps_serial_number = ?
                and tm.transfer_to = '$this->sbrunch'
            ", [$data->serial])->result();

        if (count($transferReport) > 0) {
            foreach ($transferReport as $key => $value) {
                $res['transferHistory'] = $this->db->query("SELECT
                            trh.*,
                            br.Brunch_name as branchFrom,
                            br1.Brunch_name as branchTo
                        FROM tbl_transferserial_history trh
                        LEFT JOIN tbl_brunch br ON br.brunch_id = trh.transfer_from
                        LEFT JOIN tbl_brunch br1 ON br1.brunch_id = trh.transfer_to
                        WHERE trh.ps_serial_number = ?", $value->ps_serial_number)->result();
            }
        }

        $res['transferReport'] = $transferReport;
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            // customer payment 
            if ($data->sales->isInstallment == 1) {
                $this->db->query("delete from tbl_customer_payment where sale_id = ?", $salesId);
                ...
                for ($i = 1; $i <= $data->sales->installmentMonth; $i++) {
                    ...
                    $customerPayment = array(
                        'CPayment_invoice'         => $this->mt->generateCustomerPaymentCode(),
                        'CPayment_date'            => $date,
                        'CPayment_customerID'      => $data->sales->customerId,
                        'CPayment_TransactionType' => 'CR',
                        'CPayment_amount'          => $data->sales->installmentAmount,
                        'CPayment_status'          => 'p',
                        'CPayment_Addby'           => $this->session->userdata("FullName"),
