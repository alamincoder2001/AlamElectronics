...
class Products extends CI_Controller {
    ...
    public function addProduct() {
        ...
        try{
            ...
            $product['Product_WholesaleRate'] = $productObj->Product_SellingPrice;
            $product['status'] = 'a';
    ...
    public function getTotalStock(){
        ...
        if(isset($data->is_color_stock)){
            $stock = array_values(array_filter($stock, function($pro){ 
                    return $pro->current_quantity != 0;
                }
            ));
            ...
            $stock = array_map(function($product){
                $product->colors = $this->db->query(
                    "SELECT c.color_SiNo,
                    c.color_name,
                    count(sl.ps_id) as color_stock

                FROM tbl_product_serial_numbers  as sl
                join tbl_color c on c.color_SiNo = sl.color_id
                WHERE sl.ps_brunch_id = '$this->brunch'
                AND sl.ps_status = 'a'
                AND sl.ps_p_r_status <> 'yes' AND (sl.ps_s_status IS NULL OR sl.ps_s_status <> 'yes'  OR sl.ps_s_r_status ='yes')
                AND sl.ps_prod_id = '$product->Product_SlNo'
                group by sl.color_id
                ")->result();
                return $product;
            }, $stock);
        }
    ...
    public function getSerialByProd(){
        ...
        if(isset($data->categoryId) && $data->categoryId != '') {
            $clauses .= " AND prod.ProductCategory_ID = $data->categoryId";
        }
        
        if(isset($data->color_id) && $data->color_id != '') {
            $clauses .= " AND sl.color_id = $data->color_id";
        }
        ...
        $serials = $this->db->query("
            SELECT 
                ...
                prod.Product_Code
            FROM tbl_product_serial_numbers  as sl
            INNER JOIN tbl_product as prod ON prod.Product_SlNo = sl.ps_prod_id
            left join tbl_productcategory pc on pc.ProductCategory_SlNo = prod.ProductCategory_ID
            left join tbl_color c on c.color_SiNo = sl.color_id
    ...
    public function serial_stock()  {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Serial Stock";
        $data['content'] = $this->load->view('Administrator/stock/serial_stock', $data, TRUE);
        $this->load->view('Administrator/index', $data); 
    }
