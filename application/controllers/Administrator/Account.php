...
class Account extends CI_Controller {
    ...
    public function getAccounts(){
        ...
        if(isset($data->investment)) {
            $investClause = " and is_investment = '$data->investment'";
        }
    ...
    function getOtherIncomeExpense(){
        ...
        if(isset($data->dateFrom) && $data->dateFrom != '' && isset($data->dateTo) && $data->dateTo != ''){
            ...
            $paymentClause .= " and cp.CPayment_date between '$data->dateFrom' and '$data->dateTo'";
        }

        if(isset($data->customer) && $data->customer != null){
            $paymentClause .= " and cp.CPayment_customerID = '$data->customer'";
        }
...
// ) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;
    ...
    public function getBankAccounts(){
        ...
        $loanClause = "";
        if(isset($data->isLoan)) {
            $loanClause = " and is_loan_account = '$data->isLoan'";
        }
    ...
    public function getBankTransactions(){
        ...
        $loanClause = "";
        if(isset($data->isLoan)){
            $loanClause = " and bt.is_loan_transaction = '$data->isLoan'";
        }
        ...
        $transactions = $this->db->query("
            ...
            where bt.status = 1
            and bt.branch_id = ?
            $accountClause $dateClause $typeClause $loanClause
    ...
    public function loanView() {
        ...
        $loanSumamry = $this->mt->bankLoanSummary();

        $data['loan_balance'] = array_reduce($loanSumamry, function($prev, $curr) {
            return $prev + $curr->balance;
        });

        $data['loan_summary'] = $loanSumamry;
