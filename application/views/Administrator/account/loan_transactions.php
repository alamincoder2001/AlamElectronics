                    ...
                    <div class="col-md-6 col-md-offset-1">
                        <form action="" class="form-horizontal" @submit.prevent="saveTransaction">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-8">
                                    <select class="form-control" v-model="transaction.transaction_type" required style="padding:0px;">
                                        ...
                                        <option value="withdraw">Receive Loan</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-8">
                                    <input type="number" step="0.01" class="form-control" v-model="transaction.amount" required>
                                </div>
                            </div>
                            ...
                            <datatable :columns="columns" :data="transactions" :filter-by="filter">
                                <template scope="{ row }">
                                    <tr v-bind:class="[row.transaction_type == 'deposit' ? 'transaction-deposit' : 'transaction-withdraw']">
                                        ...
                                        <td>{{ row.bank_name }}</td>
                                        <td>{{ row.transaction_type == 'deposit' ? 'Deposit' : 'Loan Received' }}</td>
                                        <td>{{ row.note }}</td>
                                        <td>{{ row.amount }}</td>
                                        <td>{{ row.saved_by }}</td>
                                        <td>
...
<script>
    ...
    new Vue({
        ...
        methods: {
            getAccounts(){
                axios.post('/get_bank_accounts', { isLoan: true })
                .then(res => {
            ...
            saveTransaction(){
                ...
                this.transaction.is_loan_transaction = true;
