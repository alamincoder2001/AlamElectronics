				...
				<div class="modal-wrapper">
					<div class="modal-container">
						...
						<div class="modal-footer">
							<slot name="footer">
								<button class="modal-default-button" @click="serialHideModal" style="border: none;font-size: 18px;color: white;">
									Close
								</button>
							</slot>
						</div>
	...
	<div class="row">
		<div class="col-md-6 col-md-offset-3">
			...
			<table class="table table-bordered">
				<tr>
					<td>Product Name : </td>
					<td>{{serialReport.product ? serialReport.product.Product_Name : ''}}</td>
				</tr>
				<tr>
					<td>Product Code : </td>
					<td>{{serialReport.product ? serialReport.product.Product_Code : ''}}</td>
				</tr>
				<tr>
					<td>Product Color : </td>
					<td>{{serialReport.product ? serialReport.product.color_name : ''}}</td>
				</tr>
				<tr>
					<td>Category : </td>
					<td>{{serialReport.product ? serialReport.product.ProductCategory_Name : ''}}</td>
				</tr>
			</table>
		</div>
		<div class="col-md-6">
			...
			<table class="table table-bordered">
				...
				<tr v-for="(serial, sl) in serialReport.purchaseReport">
					<td>Return: </td>
					<td :style="{color: serial.ps_p_r_status == 'yes' ? 'red' : ''}">{{ serial.ps_p_r_status }}</td>
				</tr>
				<tr v-for="(serial, sl) in serialReport.purchaseReport">
					<td>Return Date: </td>
					<td>{{ serial.PurchaseReturn_ReturnDate }}</td>
				</tr>
			</table>
		</div>
		...
		<div class="col-md-6">
			...
			<table class="table table-bordered">
				...
				<tr v-for="(serial, sl) in serialReport.saleReport">
					<td>Return: </td>
					<td :style="{color: serial.ps_s_r_status == 'yes' ? 'red' : ''}">{{ serial.ps_s_r_status }}</td>
				</tr>
				<tr v-for="(serial, sl) in serialReport.saleReport">
					<td>Return Date: </td>
					<td>{{ serial.SaleReturn_ReturnDate }}</td>
				</tr>
			</table>
		</div>
		<div class="col-md-6">
			...
			<table class="table table-bordered">
				<tr v-for="(serial, sl) in serialReport.transferReport">
					<td>Transfer Date : </td>
					<td>{{ serial.transferDate }}</td>
				</tr>
				<tr>
					<th>From Branch</th>
					<th>To Branch</th>
				</tr>
				<tr v-for="(item, index) in serialReport.transferHistory">
...
<script>
	...
	new Vue({
		...
		methods: {
			...
			async getStock() {
				await axios.post('/get_serial_report', {
						serial: this.selectedSerial.ps_serial_number
					})
					.then((res) => {
