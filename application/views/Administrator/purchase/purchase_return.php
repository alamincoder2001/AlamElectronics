			...
			<table class="table table-bordered">
				...
				<tbody>
					<template v-for="(product, sl) in cart">
						...
						<tr v-for="(pur, sl) in product.serial.filter(item => item.ps_s_r_status == 'no')">
							<td>{{ sl + 1 }}</td>
							<td>{{ product.Product_Name }}({{ pur.ps_serial_number }})</td>
							<td>1</td>
							<td>{{ pur.purchase_rate }}</td>
							<td>{{ product.returned_quantity }}</td>
							<td>{{ product.returned_amount }}</td>
							<td><a href="" class="btn rt-btn btn-sm"  @click.prevent="serialProductReturn(pur)">Return</a></td>
							<td>{{ pur.purchase_rate }}</td>
							<td></td>
						</tr>
					</template>
...
<script>
	...
	new Vue({
		...
		methods:{
			...
			calculateTotal(){
				let returnTotal = this.cart.reduce((prev, cur) => {return prev + (cur.return_amount ? parseFloat(cur.return_amount) : 0.00)}, 0);
				let serialReturnTotal = this.serialCart.reduce((prev, cur) => {return prev + (cur.returnAmount ? parseFloat(cur.returnAmount) : 0.00)}, 0);
			...
			serialProductReturn(product) {
				let serial = {
					...
					returnAmount: product.purchase_rate,
					quantity: 1
				}
