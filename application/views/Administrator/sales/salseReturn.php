						...
						<tbody>
							<template v-for="(product, sl) in cart">
								...
								<tr v-else v-for="(sel, sl) in product.serial.filter(item => item.ps_s_r_status == 'no')">
									...
									<td>1</td>
									<td>{{ sel.Product_SellingPrice }}</td>
									<td>{{ product.returned_quantity }}</td>
									<td>{{ product.returned_amount }}</td>
...
<script>
	...
	new Vue({
		...
		methods:{
			...
			calculateTotal(){
				let cartReturn = this.cart.reduce((prev, cur) => {return prev + (cur.return_amount ? parseFloat(cur.return_amount) : 0.00)}, 0);
				let serialReturn = this.serialCart.reduce((prev, cur) => {return prev + (cur.returnAmount ? parseFloat(cur.returnAmount) : 0.00)}, 0);
